...
The `authController.js` manages user authentication and authorization for the application. It handles the processes of user registration, login, and token generation. Key functionalities include validating user credentials, creating and issuing JSON Web Tokens (JWTs) upon successful authentication, and managing user sessions. The controller makes sure of secure access to protected resources by verifying tokens and user permissions. It also handles errors related to authentication failures and provides appropriate responses for unsuccessful login attempts or unauthorized access.

2. `orderController`

This controller manages order operations for a database. It includes methods for retrieving, creating, and deleting orders. 
- `getOrders` method retrieves all orders, formatting their data for display purposes. - `getOrderDetails` method fetches details of a specific order, including associated parts and their quantities. 
- `createOrder` method handles the creation of a new order, calculates costs and delivery estimates, and updates part availability. 
- `deleteOrder` method deletes an order and restores the availability of parts previously included in the order. 
Utility functions include:
- `addBusinessDays` which calculates a future date excluding weekends, and 
- `calculateTotalTime`, which computes the total time required for an order based on work hours and delivery time.
...
- Check `/backend/data` folder, should be empty to enable script from `database.js` file to populate it with example db, but can be populated with test dbs if user ran tests earlier.
